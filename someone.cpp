...
{
  while ( handCards.size() ) departedCards<<handCards.takeLast();
}

void Player::shuffleDepartedCardsIntoDeck( void )
{
  while ( departedCards.size() ) deck<<departedCards.takeLast();
  shuffleDeck();
}
...
void Player::shuffleDeck( void )
{
  ...
  while ( deck.size() ) cards<<deck.takeAt( random( 0, deck.size()-1 ) );
  deck = cards;
}
...
void Monster::chooseSkill()
{
  ...
  {
    chances<<skill[ i ]->getChance();
    sumOfChance += chances.last();
  }
  r = random( 1, sumOfChance );
  for ( int i = 0; i<skill.size() && i<chances.size(); i++ )
  {
    if ( r<chances[ i ] )
    {
      nextSkill = skill[ i ];
